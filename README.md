...
有一个或多个占用内存较多的程序，每个程序监听一个端口提供 TCP 服务。门卫程序先接管这些端口（release 二进制约 292KB，运行 5 个端口时 RSS 约 4.3MB），当有人访问某个端口时，页面显示 **"xxx 启动中"**，自动启动对应的后端程序，然后释放端口让后端程序接管。
...
- **极简内存占用**：release 二进制约 292KB，每端口线程栈 512KB（可配置），100 端口 RSS 约 6.3MB
- **端口分组**：`group` 字段将多个端口归入同一组，启动后端时按组协调，避免重复启动
- **多平台编译**：Linux（x86_64 / ARM64 / i386 / RISC-V）、macOS（Apple Silicon）、Windows（MinGW / MSVC）
...
1. **门卫监听** `listen` 端口，占用极少内存（release 二进制约 292KB，RSS 约 4.3MB/5端口，每增加 1 端口 RSS 增加约 20KB；每端口线程栈 512KB（可配置），100 端口 RSS 约 6.3MB）
