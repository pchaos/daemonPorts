...
int main(int argc, char* argv[]) {
    ...
    if (g_controlConfig.auth.type == "token" && g_controlConfig.auth.token.empty()) {
        g_controlConfig.auth.token = platform::generateAuthToken();
        if (g_controlConfig.auth.token.empty()) {
            std::cerr << "[ControlServer] 错误: 无法生成安全认证令牌 (CSPRNG 不可用)，拒绝启动" << std::endl;
            return 1;
        }
    ...
    if (g_controlServer.isEnabled()) {
        ...
        if (g_controlConfig.auth.type == "token") {
            if (tokenAutoGenerated) {
                ...
                std::cout << "[ControlServer] 认证令牌: 已从配置加载（不打印）" << std::endl;
            }
        } else {
